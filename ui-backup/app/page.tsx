...
export default function Home() {
  ...
  const preloadSnippets = async (codeMode: 'curated' | 'new' = mode, targetCount: number = 10) => {
    ...
    // Stop preloading if we already have 75+ snippets in cache
    if (snippetCache.length >= 75) {
      console.log(`[Client] Cache is full (${snippetCache.length} snippets), skipping preload`)
      return
    }
    
    setIsPreloading(true)
    try {
      const currentCacheSize = snippetCache.length
